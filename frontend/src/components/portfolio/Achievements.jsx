...
// Fallback static data when no awards exist in database
    ...
    const fetchAwards = async () => {
      try {
        ...
        if (response.data.data && response.data.data.length > 0) {
          ...
          // Use fallback data if no awards in database
          ...
          {achievements.map((achievement, index) => (
            ...
            >
              ...
              <div className="relative z-10 flex-shrink-0">
                <div
                  className="w-14 h-14 rounded-full bg-dark-200 border-2 border-primary 
              ...
              {/* Content Card */}
              <div
                className="flex-1 bg-dark-100 rounded-xl p-6 hover:bg-dark-200 
                           transition-colors group"
              >
                <div className="flex flex-wrap items-start justify-between gap-4 mb-3">
                  <div>
                    <h3
                      className="text-xl font-display font-semibold text-light 
                  ...
                  <span
                    className={`px-4 py-1.5 rounded-full text-sm font-medium ${getTypeStyles(
                      achievement.type
                    )}`}
                  >
