...
import { HiChevronLeft, HiChevronRight, HiX, HiShare, HiMail } from 'react-icons/hi';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[70vh]">
        ...
        <div className="p-6 lg:p-8 overflow-y-auto">
          ...
          <div className="mb-6">
            ...
            {artwork.year && (
              <p className="text-light-300">{artwork.year}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {artwork.medium && (
              <div>
                <span className="text-xs text-dark-400 uppercase tracking-wider">Medium</span>
                <p className="text-light capitalize">{artwork.medium}</p>
              </div>
            )}
            {artwork.surface && (
              <div>
                <span className="text-xs text-dark-400 uppercase tracking-wider">Surface</span>
                <p className="text-light capitalize">{artwork.surface}</p>
              </div>
            )}
            {artwork.dimensions && (
              <div>
                <span className="text-xs text-dark-400 uppercase tracking-wider">Dimensions</span>
            ...
            {artwork.category && (
              <div>
                <span className="text-xs text-dark-400 uppercase tracking-wider">Category</span>
          ...
          {artwork.isForSale && (
            <div className="p-4 bg-dark-200 rounded-xl mb-6">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xs text-dark-400 uppercase tracking-wider">Price</span>
          ...
          {artwork.description && (
            <div className="mb-6">
              <h3 className="text-sm font-medium text-light mb-2">About this piece</h3>
              <p className="text-light-300 leading-relaxed">{artwork.description}</p>
            </div>
          )}
          ...
          {artwork.competition?.name && (
            <div className="p-4 bg-primary/10 border border-primary/30 rounded-xl mb-6">
              <span className="text-xs text-primary uppercase tracking-wider">Competition Entry</span>
              <p className="text-light font-medium">{artwork.competition.name}</p>
              {artwork.competition.award && (
                <p className="text-primary text-sm">{artwork.competition.award}</p>
              )}
            </div>
          )}
